...
export default function WalletHomePage() {
  ...
  const pendingInboxTable = usePagedFilter(pendingInbox, { pageSize: 5, match: (consent, q) => { const r = consent as unknown as Record<string, unknown>; return [String(r.fiId ?? r.requestedByFiId ?? ''), String(r.purpose ?? ''), String(r.subjectUserId ?? r.userId ?? ''), String(r.status ?? r.lifecycleStatus ?? '')].join(' ').toLowerCase().includes(q); } });
    ...
    <div className="space-y-5 text-slate-800">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 text-slate-800 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-600">Wallet Overview</p>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-[#f8faff] p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-600">Token</p>
              <div className="mt-2 flex items-center justify-between gap-3">
                <div>
                  <p className="text-lg font-semibold text-blue-700">{registrySnapshot?.status ?? 'UNKNOWN'}</p>
                  <p className="mt-1 text-xs text-slate-500">Expiry: {tokenExpiryLabel}</p>
                </div>
                <button
            ...
            <div className="rounded-2xl border border-slate-200 bg-[#f8faff] p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-600">Audit signal</p>
              <div className="mt-2">
                <p className="text-lg font-semibold text-emerald-700">{lastEvent?.label ?? 'No activity yet'}</p>
                <p className="mt-1 text-xs text-slate-500">Latest event from {lastEvent?.service ?? '—'}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-4">
          <DashboardKpiCard label="Pending approvals" value={pending} hint="Inbox + delegated" tone="amber" Icon={Clock} />
          <DashboardKpiCard label="Approved consents" value={approved} hint="Reusable access trail" tone="emerald" Icon={BadgeCheck} />
          <DashboardKpiCard label="Rejected / revoked" value={rejected} hint="User-controlled sharing" tone="rose" Icon={CheckCircle2} />
          <DashboardKpiCard label="Active delegations" value={activeDelegations} hint="Nominee & guardian" tone="indigo" Icon={Users2} />
        </div>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr]">
        <div className="space-y-5">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 text-slate-800">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">Shortcuts to the key workflows you’ll demo.</p>
              </div>
            ...
            <div className="mt-4 grid gap-3 lg:grid-cols-2">
              <Link to="/wallet/nominees" className="rounded-2xl border border-slate-200 bg-[#fafbff] p-4 hover:bg-[#f6f8fc]">
                <p className="text-sm font-semibold">Nominees</p>
                <p className="mt-1 text-xs text-slate-600">Create/disable nominees and keep them auditable.</p>
              </Link>
              <Link to="/wallet/delegations" className="rounded-2xl border border-slate-200 bg-[#fafbff] p-4 hover:bg-[#f6f8fc]">
                <p className="text-sm font-semibold">Delegations</p>
                <p className="mt-1 text-xs text-slate-600">Enable delegated approvals only from active nominees.</p>
              </Link>
              <Link to="/command/registry" className="rounded-2xl border border-slate-200 bg-[#fafbff] p-4 hover:bg-[#f6f8fc]">
                ...
                <p className="mt-1 text-xs text-slate-600">See token lifecycle, expiry, and supersession chain.</p>
              </Link>
              <Link to="/command/audit" className="rounded-2xl border border-slate-200 bg-[#fafbff] p-4 hover:bg-[#f6f8fc]">
                ...
                <p className="mt-1 text-xs text-slate-600">Evidence chain across FI → wallet → registry.</p>
              </Link>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 text-slate-800">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Consent inbox preview</p>
                <p className="mt-1 text-sm text-slate-600">Latest requests requiring action.</p>
              </div>
              <Link to="/wallet/inbox" className="text-xs font-semibold text-slate-700 hover:text-slate-700 hover:underline">
                View all
              </Link>
            </div>

            <div className="mt-4">
              <TableSearchPager {...pendingInboxTable} placeholder="Search FI / purpose / user / status" />
            </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {pendingInboxTable.paged.length === 0 ? (
                    <tr>
                    ...
                    pendingInboxTable.paged.map((consent) => {
                      ...
                      return (
                        <tr key={String(record.consentId ?? record.id)} className="odd:bg-white even:bg-[#fbfcff] hover:bg-[#f3f7ff]">
                          <td className="px-4 py-3 text-slate-800">{fi}</td>
                          <td className="px-4 py-3 text-slate-700">{purpose}</td>
                          <td className="px-4 py-3 text-slate-600">{subjectUser}</td>
                          <td className="px-4 py-3">
                            <span
                              className={
                                isDelegated
                                  ? 'inline-flex rounded-full border border-indigo-200 bg-indigo-50 px-2 py-1 text-xs font-semibold text-indigo-700'
                                  : 'inline-flex rounded-full border border-amber-300/30 bg-amber-300/10 px-2 py-1 text-xs font-semibold text-amber-700'
                              }
                            >
        ...
        <div className="space-y-5">
          <HomePlannerPanel
            ...
            items={[
              { title: 'Approve consent inbox', time: 'Today · Wallet Inbox', badge: String(pending) },
              { title: 'Renew token check', time: 'This week', badge: tokenExpiryLabel },
              { title: 'Delegation review', time: 'This week', badge: String(activeDelegations) },
            ]}
          />
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 text-slate-800">
            ...
            <div className="mt-4 space-y-3">
              {nextActions.length === 0 ? (
                <div className="rounded-2xl border border-slate-200 bg-[#fafbff] p-4 text-sm text-slate-600">
                  You’re all set — no urgent actions.
                </div>
              ) : (
                nextActions.map((item) => (
                  <div key={item.title} className="rounded-2xl border border-slate-200 bg-[#fafbff] p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {item.ctaLabel ? (
                        item.ctaPath ? (
                          <Link
                            to={item.ctaPath}
                            className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-[#fafbff] px-3 py-2 text-xs font-semibold text-slate-800 hover:bg-white/[0.1]"
                          >
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 text-slate-800">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">KYC tips (AI-assisted)</p>
            <div className="mt-4 space-y-3 text-sm">
              <div className="rounded-2xl border border-slate-200 bg-[#fafbff] p-4">
                <p className="font-semibold text-slate-800">Selective disclosure</p>
                <p className="mt-1 text-xs text-slate-600">Approve only required fields (e.g., address without DOB) to reduce data exposure.</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-[#fafbff] p-4">
                <p className="font-semibold text-slate-800">Delegation controls</p>
                <p className="mt-1 text-xs text-slate-600">Use nominee delegation for legal heirs/guardians with expiry + purpose restrictions.</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-[#fafbff] p-4">
                <p className="font-semibold text-slate-800">Lifecycle hygiene</p>
                <p className="mt-1 text-xs text-slate-600">Renew tokens before expiry to keep onboarding frictionless across institutions.</p>
              </div>
            </div>
          </div>

          <div className="rounded-3xl border border-slate-200 bg-white p-5 text-slate-800">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Health snapshot</p>
